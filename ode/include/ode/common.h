 ...
 */
/* TODO: contrary to the comment above, these are NOT private anymore, since we
 * now support replacing the ODE collision detection by a custom one.
 */

#define ODE_USE_FUNCTION_POINTERS 1 /* AB: TODO: remove. this is currently for testing, so we can easily see what was changed for function pointers */

#if !ODE_USE_FUNCTION_POINTERS
void dGeomMoved (dGeomID);
dGeomID dGeomGetBodyNext (dGeomID);
/*
 * AB: dGeomSetBody() is declared in collision.h, but actually is
 * required here (i.e. in non-collision detection API), too, because
 * dBodyDestroy() calls it!
 */
#else /* ODE_USE_FUNCTION_POINTERS */

extern void (*dGeomMoved)(dGeomID);
extern dGeomID (*dGeomGetBodyNext)(dGeomID);

/*
 * set all function pointers required to replace the default ODE collision
 * detection by a custom collision detection
 */
ODE_API void dChangeCollisionDetection(void (*dGeomMoved)(dGeomID),
              dGeomID (*dGeomGetBodyNext)(dGeomID),
              void (*dGeomSetBody)(dGeomID, dBodyID),
              dBodyID (*dGeomGetBody)(dGeomID));
#endif /* ODE_USE_FUNCTION_POINTERS */
